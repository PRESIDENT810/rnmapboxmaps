...
public class RNMBXMarkerView: UIView, RNMBXMapComponent {
  ...
  func getAnnotationManager(mapView_: MapView) -> ViewAnnotationManager {
    mapView_.viewAnnotations
  }
  ...
  private func add() {
    self.map?.withMapView(callback: {mapView_ in
      let annotationManager = self.getAnnotationManager(mapView_: mapView_)
